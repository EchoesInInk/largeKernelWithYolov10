...
ultralytics
cfg
里面存储着各个配置文件。 请注意，cfg中有一个配置文件叫做default.yaml，里面集成了训练选项和训练参数，如果要重新训练的话请注意里面的描述。

datasets
里面存储着数据集配置文件。 格式可以按照ImageNet.yaml进行修改。如果要在训练中调用，请确保配置文件在这个文件夹里。

models
存储着模型配置文件。 模型在下面的v10文件夹里，名字叫LK_YOLOv10。
